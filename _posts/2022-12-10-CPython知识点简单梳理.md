...
## 1.20 不可变序列
str、bytes、set均是不可变序列，创建后无法被修改。bytes和str的`replace()`实际是创建了一个新的copy副本。
...
## 2.16 编译过程
通过词法解析将文本转换为CST，通过语法解析将CST转换为AST，通过`compile.c`中的`_PyAST_Compile()`将AST转换为`codeobject`。
**编译器(compiler)**:将AST转换为codeobject的过程。
