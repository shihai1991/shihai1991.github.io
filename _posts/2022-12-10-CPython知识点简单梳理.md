...
```python
a = '中'
# 下方两个函数输出的均是1，这个1是指unicodeobject的长度
len(a)
a.__len__()
```
...
```
实际的存储业务逻辑：`PyUnicode_FromStringAndSize()`->`PyUnicode_DecodeUTF8Stateful()`->`unicode_decode_utf8()`->`_PyUnicodeWriter_WriteCharInline(&writer, ch)`->`xxxlib_utf8_decode()`，此处的ch字符串是第一个字符的十进制的unicode编码。   
实际在生成unicodeobject的最后阶段，有一段函数处理逻辑会把`strlen()`计算出来的实际长度刷新为unicode的个数长度（如：这个示例字符串实际长度是6，会被刷新为2）[代码链接](https://github.com/python/cpython/blob/36a0b1d0dddbdf324d98071ea31e7e9151eea6d5/Objects/unicodeobject.c#L13115)。
...
## 1.11 staticmethod和classmethod的区别
