...
把所有的输入词向量放到一个矩阵$X$中，然后分别和3个权重矩阵$W^Q$、$W^K$和$W^V$相乘，这三个权重矩阵就是大模型在预训练阶段时，通过神经网络反向传播训练出来的权重矩阵，这里的权重指的是神经网络中的连接权重，得到$Q$,$K$和$V$矩阵。这里引入三个权重矩阵是为了对$X$进行线性变换，提升模型的拟合能力。其中$d_k$是Key向量的长度，用softmax进行归一化处理，最终矩阵相乘获得最后的Self-Attention输出。  
