...
如果用IDE执行就不会报错，因为IDE会使用`-cp`引用编译后的class文件，不会有动态加载class找不到import相关class的情况。在linux系统下可以用下面的命令方式编译groovy文件，当然如果有些文件是配置文件可能就需要手动拷贝到具体存放编译文件的路径下，此处的编译文件存放路径是$WORKSPACE/out。
```
