...
GroovyClassLoader可以通过`parseClass()`解析文件，通过此方式在缓存中就会缓存这个信息，这样重新解析相同文件就不会反复编译。 
另外一个需要注意的是`parseClass()`比较适合解析脚本，如果是嵌套类或者接口就不适合用此方式解析，因为知会返回第一个类或者接口。[原因](https://issues.apache.org/jira/projects/GROOVY/issues/GROOVY-11117?filter=allopenissues)  
另外用此方式收集嵌套接口或者类信息，会导致另外的脚本执行`import`接口操作发生变化，具体根因不清，从表面现在看是由于解析第一个内部类并返回导致的，因为`import`的就是第一个内部类。
