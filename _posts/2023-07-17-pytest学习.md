...
`Function`对象实例是在`Session`实例化过程中生成了所有的`Function`对象，这个执行动作是由`main.pytest_collection()`函数触发，而真正的遍历及生成`item`或`Function`实例对象的函数是`Session.genitems()`，比如我要执行一个test.py文件，则会触发`session.genitems("test.py")`并创建出所有的Function对象实例。
```python
