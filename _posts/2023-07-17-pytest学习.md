...
`class Function`是对测试函数的封装，所有的准备及执行动作都在这个类实例中完成，如：通过参数找fixtures的过程就是在`def setup()`中执行完成的。而实际推测需要调用的Fixture相关信息其实也在`Function`对象实例中，并在构造`TopReuqest`对象实例时将`Function`对象实例注入其中。
```python
