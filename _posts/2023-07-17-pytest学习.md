...
## 插件加载
用conftest.py文件定义pytest_plugins属性就可以让pytest在执行用例之前加载相关插件，[参考文档](https://docs.pytest.org/en/latest/reference/reference.html#globalvar-pytest_plugins)。对应的实际处理逻辑是：pytest的`PluginManager`定义了一个[`import_plugin`](https://github.com/pytest-dev/pytest/blob/bf451d47a1b3be80a7f89b3076e4816c47390037/src/_pytest/config/__init__.py#L760)可以用来加载插件。
