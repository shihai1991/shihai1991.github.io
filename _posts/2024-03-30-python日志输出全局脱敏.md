...
## 通过root logger进行配置
Filters不像Levels和Handlers，通过顶层root logger配置好，子类就能继承下来。配置Filters有两种方式，一种就是直接对Logger实例调用`setFilter()`函数添加Filters，而另一种是给`root logger`添加Handlers，然后在Handler里面通过调用`SetFilter()`函数添加Filters。  
